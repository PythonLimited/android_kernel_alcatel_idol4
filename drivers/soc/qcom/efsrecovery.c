...
char  g_reason[MAX_SSR_REASON_LEN];
//[FEATURE]-ADD-END by TCTSH.lijuan.wu, mini log, task-1488899
			...
			loff_t *pos)
{
	...
	*pos = 0;
	//[FEATURE]-ADD-BEGIN by TCTSH.lijuan.wu, mini log, task-1488899
	ret= simple_read_from_buffer(buf, count, pos, g_reason, strlen(g_reason));
	//[FEATURE]-ADD-END by TCTSH.lijuan.wu, mini log, task-1488899
...
void efsrecovery_check(char * reason)
{
	...
	//[FEATURE]-ADD-BEGIN by TCTSH.lijuan.wu, mini log, task-1488899
	memset(g_reason,0,MAX_SSR_REASON_LEN);
	strlcpy(g_reason, reason, strlen(reason)<MAX_SSR_REASON_LEN ? strlen(reason): MAX_SSR_REASON_LEN);
	pr_debug(" g_reason is %s",g_reason);
	//[FEATURE]-ADD-END by TCTSH.lijuan.wu, mini log, task-1488899
}
