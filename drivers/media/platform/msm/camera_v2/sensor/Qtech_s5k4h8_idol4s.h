...
* ------------------------------------------------------------------
xiaoming.hu@tcl.com for ido4s s5k4h8
* ------------------------------------------------------------------
*/
...
static int32_t s5k4h8_write_i2c(uint32_t addr, uint16_t data)
{
    int32_t rc = -EFAULT;
	if (!s5k4h8_g_client)
        printk("FFFF OTP: s5k4h8 client null\n");
    else{
...
static void s5k4h8_write_lsc(void)
{
	...
	printk(" write lsc before stream on %s\n",__func__);
	s5k4h8_write_i2c_word(0x0B00,0x0180);
}
